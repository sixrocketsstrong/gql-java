...
public class GraphQLJavaUtils {
		...
		selSet.getSelectionSet().getSelections().stream().forEach(selection -> {
			if (selection instanceof NamedNode) {
				if (selection instanceof FragmentSpread) {
					...
					fragments.stream().filter(fd -> fd.getName().equals(spread.getName())).findFirst().ifPresent(fDef -> {
						Fragment fragment = GQL.fragment("f" + UUID.randomUUID().toString().replace("-", ""),
								typeString(fDef.getTypeCondition()));
						addSelections(fragment, fDef, def, fragments);
						dest.fragment(fragment);
					});
				} else {
					com.sixrockets.gql.Field<?> parentSel = dest.selectWith(((NamedNode<?>) selection).getName());
					...
					if (selection instanceof SelectionSetContainer) {
						...
						if (container.getSelectionSet() != null && !container.getSelectionSet().getSelections().isEmpty()) {
				...
				Fragment fragment = GQL.fragment("f" + UUID.randomUUID().toString().replace("-", ""), typeString(inlineFragment.getTypeCondition()));
	...
	public static void addArguments(FieldsAndVariables dest, Field field, OperationDefinition def) {
		field.getArguments().stream().forEach(arg -> {
			if (arg.getValue() instanceof VariableReference) {
				VariableReference value = (VariableReference) arg.getValue();
				Optional<VariableDefinition> varDef = def.getVariableDefinitions().stream()
						.filter(d -> d.getName().equals(value.getName()))
						.findFirst();
				varDef.ifPresent(d -> dest.variable(arg.getName(), fromType(d.getType())));
			}
		});
	}
	...
	public static com.sixrockets.gql.Type fromType(Type type) {
		if (type instanceof NonNullType) {
			return com.sixrockets.gql.Type.notNull(typeString(((NonNullType) type).getType()));
		} else {
